...
class DataValidation:
   ...
   #--------------------------------------------------------------------------------------#
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            required_columns = [column for column in self._schema_config["columns"]]
   ...
   #--------------------------------------------------------------------------------------#
    ...
    def validate_numeric_columns(self, dataframe: pd.DataFrame) -> bool:
        """
        Validates if all columns in the dataframe are numeric.
        """
        try:
            logging.info(f"Starting numeric column validation for dataframe with {len(dataframe.columns)} columns.")
   ...
   #--------------------------------------------------------------------------------------#
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f"Train dataframe contains non-numeric columns. \n"

            status = self.validate_numeric_columns(dataframe=test_dataframe)
            
            if not status:
                error_message = f"Test dataframe contains non-numeric columns. \n"

            # Lets check for datadrift
